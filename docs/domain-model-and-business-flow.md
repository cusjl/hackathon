...
成功登录的 `LoginVO` 会返回稳定的 `userId`、当前账号 `email`，以及学生真实的 `casId`（外部账号为 `null`）。`casId` 不等于本次登录输入：无论学生以学号、手机号还是邮箱登录，返回的都是学生档案中的真实学号。前端需要判断队长时，应优先使用队伍详情 `members[].leader`；如需比对当前成员身份，则使用 `userId` 与 `members[].userId`。
...
- 队伍协同类型是精确的三档约束：`同专业` 要求所有成员的专业和校区均一致；`跨专业` 允许专业不同，但所有成员校区必须一致；`跨校区` 对专业和校区都不设一致性限制。
- 选手按队名检索时，结果中的 `joinable` 是上述专业/校区约束的预检结果；`false` 时 `unavailableReason` 为 `专业不一致` 或 `校区不一致`。实际加入会再次执行相同校验并返回对应的 `3114 TEAM_MAJOR_CONFLICT` 或 `3110 TEAM_CAMPUS_CONFLICT`，因此检索结果不能替代最终入队请求。
...
- 未组队选手还可按队名模糊检索可加入队伍；结果自动限制为本人报名赛道内、状态有效且未满员的队伍，并返回专业/校区兼容性预检字段，实际加入时继续校验全部规则；
...
`LoginVO.userId` 是当前账号的稳定平台主键；`LoginVO.email` 是账号绑定邮箱；学生的 `LoginVO.casId` 始终是学生档案中的真实学号，外部账号则为 `null`。三者语义互不复用，前端不得用邮箱或姓名推断用户身份。
